...
$(window).on("content loaded", function() {
    ...
    const CONFIG = {
		...
		searchSize: 5, // Number of files/folders to process in each batch (search results)
    ...
    /*
     * SIMPLIFIED CACHE AND FETCH PROCESS:
     * 1. On initial load, read JSON data from embedded script tag
     * 2. Store data in memory cache for faster access
     * 3. For subsequent navigation:
     *    a. Try to use in-memory cached data first (faster)
     *    b. Fall back to reading from embedded JSON if memory cache fails
     */
    ...
    // Function to read the embedded JSON data from the script tag
    function getEmbeddedJSON() {
        const jsonScript = document.getElementById('dom-cache');
    ...
    async function preloadDirectoryData() {
        try {
            // Read data from embedded JSON in the page
            const data = getEmbeddedJSON();
            ...
            if (CONFIG.memoryCache) {
                memoryCache.set('directoryData', data);
            }
            
            initialLoadComplete = true;
            return data;
        } catch (error) {
            console.error('Error loading embedded JSON data:', error);
            initialLoadComplete = true;
            throw error;
        }
    }

    // Modified fetchDirectoryData function to use memory cache or embedded JSON
    async function fetchDirectoryData() {
        ...
        if (!initialLoadComplete) {
            try {
                ...
                console.error('Failed to preload directory data:', error);
                throw error;
            }
        }
        
        // For subsequent calls, try memory cache first
        if (CONFIG.memoryCache) {
            const memoryCachedData = memoryCache.get('directoryData');
            if (memoryCachedData) return memoryCachedData;
        }
        
        // Fall back to reading from embedded JSON
        const embeddedData = getEmbeddedJSON();
        if (embeddedData) {
            // Update memory cache
            if (CONFIG.memoryCache) {
                memoryCache.set('directoryData', embeddedData);
            }
            return embeddedData;
        }
        
        throw new Error('Unable to retrieve directory data');
    }

	document.addEventListener('keydown', function(e) {
		// Only process if Enter or Spacebar is pressed
		if (e.key !== 'Enter' && e.key !== ' ') return;
	
		const target = e.target;
		// Only process if target matches our selectors
		if (!target.matches('label, h1 a, li a')) return;
	
		e.preventDefault();
	
		if (target.matches('label')) {
		// For labels, trigger click but maintain focus
    ...
    document.addEventListener('DOMContentLoaded', function() {
    	...
    	preloadDirectoryData()
            ...
            .catch(error => {
                console.error('Failed to initialize directory data:', error);
            });
    	...
    	window.isSpecialSearchActive = false;
    	...
    	searchInput.addEventListener('input', function(e) {
    		...
    		// Address bar manipulation for special search
    		let url = new URL(window.location.href);
    		// Remove all special search params first
    		url.searchParams.delete('search-all');
    		url.searchParams.delete('search-folder');
    		url.searchParams.delete('search-file');
    		if (isAnySpecialPattern) {
    			let searchTerm = '';
    			if (startsWithFolderPattern) {
    				searchTerm = this.value.substring(9).trim();
    				url.searchParams.set('search-folder', searchTerm);
    			} else if (startsWithFilePattern) {
    				searchTerm = this.value.substring(7).trim();
    				url.searchParams.set('search-file', searchTerm);
    			} else if (startsWithAllPattern) {
    				searchTerm = this.value.substring(3).trim();
    				url.searchParams.set('search-all', searchTerm);
    			}
    			// Update the address bar without reloading
    			window.history.replaceState({}, '', url.pathname + url.search);
    		} else {
    			// If not in special search, clear the special search params
    			window.history.replaceState({}, '', url.pathname);
    		}

    		// If we're already in special search mode and the input changes, cancel any ongoing batch processing
    		if (window.isSpecialSearchActive && isAnySpecialPattern) {
    		...
    		if (!window.isSpecialSearchActive && isAnySpecialPattern) {
    			window.isSpecialSearchActive = true;
    		...
    		if (isAnySpecialPattern && window.isSpecialSearchActive) {
    		...
    		} else if (!isAnySpecialPattern && window.isSpecialSearchActive) {
    			...
    			setTimeout(() => {
    				...
    				if (isAnySpecialSearch && linkElement.closest('li') && !linkElement.classList.contains('icon-directory')) {
    					...
    					if (parentDir === ".") {
    						...
    						// Find and set lastClickedItemId for the root directory
    						fetchDirectoryData().then(data => {
    							const rootContents = data.directories['root'] || data.directories['.'] || [];
    							const found = rootContents.find(entry => entry.name === lastClickedItemName);
    							window.lastClickedItemId = found && found.item_id !== undefined ? found.item_id : null;
    							const clickedItemId = window.lastClickedItemId;
    							// console.log('[DEBUG] Set lastClickedItemId for root:', window.lastClickedItemId);
    							// Clear search bar before navigation
    							resetSearchState();
    							// Navigate to root
    							resetToRoot(false, false, false, true);
    							// --- Edge case: update URL to # or #&item=X for root navigation after special search ---
    							let url = new URL(window.location.href);
    							url.search = '';
    							let hash = '#';
    							if (clickedItemId != null) {
    								hash += `&item=${clickedItemId}`;
    							}
    							// Use replaceState to ensure the hash is always present, even if it's just '#'
    							window.history.replaceState({}, '', url.pathname + url.search + hash);
    							// console.log('[DEBUG] Edge case root navigation:');
    							// console.log('  hash:', hash);
    							// console.log('  lastClickedItemId:', window.lastClickedItemId);
    							// console.log('  final URL:', url.pathname + url.search + hash);
    							// --- End edge case ---
    						});
    						return;
    					}
    					...
    					fetchDirectoryData()
    						.then(data => {
    							...
    							// Key item_id from our data storage and store as global variable
    							const dirContents = data.directories[parentDir];
    							window.lastClickedItemId = null;
    							if (dirContents && Array.isArray(dirContents)) {
    								const found = dirContents.find(entry => entry.name === clickedItemName);
    								if (found && found.item_id !== undefined) {
    							window.lastClickedItemId = found.item_id;
    								}
    							}
    							// console.log('Clicked item_id from data:', window.lastClickedItemId);
    	...
    	function displayMatchingItems(items, searchOperationId) {
    		...
    		function processNextBatch() {
    			...
    			// Process up to CONFIG.searchSize items at a time to prevent lag
    			let itemsProcessed = 0;
    			while (currentIndex < processQueue.length && itemsProcessed < CONFIG.searchSize) {
    				...
    				if (queueItem.isHeader) {
    					...
    					dirHeader.innerHTML = queueItem.content === "." ? 
    						`~ <span> / </span>` : 
    						`~ <span> / </span>${queueItem.content}<span> / </span>`;
		...
		function resetSearchState() {
			...
			// Clear any highlighted-item state left by special search result navigation
			window.lastClickedItemId = null;
			if (window.location.hash.includes('&item=')) {
				const directoryId = window.location.hash.substring(1).split('&item=')[0];
				history.replaceState(
					null,
					'',
					window.location.pathname + window.location.search + (directoryId ? `#${directoryId}` : '')
				);
			}
		...
		function logPathHistory(action, fromHashChange = false) {
			...
			if (pathHistory.length > 0) {
				return fetchDirectoryData().then((data) => {
					...
					console.groupEnd();
					if (!fromHashChange) {
						// Remove every character after the ?search in the address bar, if present
						const url = window.location.href;
						const searchIndex = url.indexOf('?search');
						if (searchIndex !== -1) {
							const newUrl = url.substring(0, searchIndex);
							history.replaceState(null, '', newUrl);
						}

						appendURLHash(lastDirId);
					}
				});
			} else {
				console.groupEnd();
		...
		async function handleInitialHash() {
			...
			if (window.location.hash) {
				...
				const hashParts = hash.split('&item=');
				const directoryId = hashParts[0];
				const itemId = hashParts.length > 1 ? decodeURIComponent(hashParts[1].trim()) : null;

				// --- Highlight item on root if hash is #&item=X ---
				if ((!directoryId || directoryId === '') && itemId) {
					// We're at root and have an item to highlight
					try {
						const data = await fetchDirectoryData();
						const rootContents = data.directories['root'] || data.directories['.'] || [];
						const found = rootContents.find(entry => String(entry.item_id) === String(itemId));
						if (found) {
							highlightAndScrollToItem('[data-path]', found.name);
						}
					} catch (error) {
						console.error('Error highlighting root item from hash:', error);
					}
				...
				try {
					...
					if (path) {
						...
						// Navigate to the folder
						return RIPE(path, true).then(() => {
							// highlight on load if &item= is present
							const hash = window.location.hash.substring(1);
							const hashParts = hash.split('&item=');
							const itemId = hashParts.length > 1 ? decodeURIComponent(hashParts[1].trim()) : null;
							if (itemId) {
								// Find the <a> element whose item_id matches
								const container = document.querySelector('[data-path]');
								if (container) {
									const items = container.querySelectorAll('li a');
									const dirData = data.directories[path] || [];
									for (const item of items) {
										const entry = dirData.find(entry => entry.name === item.getAttribute('title'));
										if (entry && String(entry.item_id) === String(itemId)) {
											highlightAndScrollToItem('[data-path]', item.getAttribute('title'));
											break;
										}
									}
								}
		...
		function handleHashChange() {
			...
			if (hash) {
				const directoryId = hash.substring(1).split('&item=')[0]; // Remove the # character and any item highlight
		...
		function handleInitialSearch() {
			...
			const startsWithFolderPattern = value.startsWith(CONFIG.folder);
			const startsWithFilePattern = value.startsWith(CONFIG.file);
			const startsWithAllPattern = value.startsWith(CONFIG.all);
			const isAnySpecialPattern = startsWithFolderPattern || startsWithFilePattern || startsWithAllPattern;
			const url = new URL(window.location.href);
			const hasAddressSearch = url.searchParams.has('search-folder') || url.searchParams.has('search-file') || url.searchParams.has('search-all');

			if (isAnySpecialPattern) {
				// Activate special search mode
				window.isSpecialSearchActive = true;

				let searchTerm, searchType;
				if (startsWithFolderPattern) {
					searchTerm = value.substring(CONFIG.folder.length).trim();
					searchType = 'folder';
				} else if (startsWithFilePattern) {
					searchTerm = value.substring(CONFIG.file.length).trim();
					searchType = 'file';
				} else {
					searchTerm = value.substring(CONFIG.all.length).trim();
					searchType = 'all';
				}

				if (typeof performSpecialSearch === 'function' && searchTerm) {
					performSpecialSearch(searchTerm, searchType);
				}

				// If ?search does not exist in the address bar, append special search to the address bar
				if (!hasAddressSearch) {
			...
			} else if (!isAnySpecialPattern && hasAddressSearch) {
                // Check which special search param is present
                let searchType = null;
                let searchTerm = '';
                if (url.searchParams.has('search-folder')) {
                    searchType = 'folder';
                    searchTerm = url.searchParams.get('search-folder');
                    searchInput.value = CONFIG.folder + searchTerm;
                } else if (url.searchParams.has('search-file')) {
                    searchType = 'file';
                    searchTerm = url.searchParams.get('search-file');
                    searchInput.value = CONFIG.file + searchTerm;
                } else if (url.searchParams.has('search-all')) {
                    searchType = 'all';
                    searchTerm = url.searchParams.get('search-all');
                    searchInput.value = CONFIG.all + searchTerm;
                }
                if (searchType && searchTerm) {
                    window.isSpecialSearchActive = true;
                    // Sync sessionStorage with the input value
                    sessionStorage.setItem('searchText', searchInput.value);
                    if (typeof performSpecialSearch === 'function') {
                        performSpecialSearch(searchTerm, searchType);
                    }
                }
            }
